...
import { setupAuth, isAuthenticated, optionalAuth, serverSupabase, getSupabaseAdmin, createAuthenticatedHandler, type AuthenticatedRequest } from "./supabaseAuth";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/health/detailed', async (req: Request, res: Response) => {
    ...
    try {
      ...
      // Check auth service
      try {
        // Try to get Supabase client - will throw if not configured
  ...
  app.get('/api/auth/user', optionalAuth, async (req: any, res: Response) => {
    ...
    try {
      ...
      try {
        ...
        const { data: supabaseUser, error: fetchError } = await admin.auth.admin.getUserById(userId);
        ...
        if (signInError) {
          ...
          if (signInError.message.includes('Invalid login credentials')) {
            ...
            while (!existingUser && attempts < maxAttempts) {
              ...
              const admin = getSupabaseAdmin();
              if (!admin) {
                console.log('⚠️  Supabase not available - cannot check user existence');
                return res.status(503).json({ 
                  message: "Authentication service unavailable",
                  code: "SERVICE_UNAVAILABLE"
                });
              }
              const { data: users, error: listError } = await admin.auth.admin.listUsers(listOptions);
  ...
  app.post('/api/auth/signup', async (req: Request, res: Response) => {
    try {
      ...
      if (!admin) {
        return res.status(503).json({ 
          message: "Sign up service temporarily unavailable. Please try again later.",
          code: "SERVICE_UNAVAILABLE"
        });
      }
      console.log('🔐 Service key available:', !!admin);
  ...
  app.get('/api/auth/verify-email', async (req: Request, res: Response) => {
    try {
      ...
      console.log('🔗 Verification callback received:', { type, hasToken: !!token_hash, next });
        ...
        const admin = getSupabaseAdmin();
        if (!admin) {
          return res.redirect(getSafeRedirectUrl(`?verified=false&error=Verification service unavailable`));
        }
