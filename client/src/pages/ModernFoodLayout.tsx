  ...
  return (
    <div data-testid="app-container" className="h-screen w-screen">
      ...
      <div data-testid="navigation-tabs" className="fixed bottom-0 left-0 right-0 bg-yellow-400 border-t border-yellow-500/60 safe-area-pb z-50 shadow-lg">
        <div className="flex items-center justify-around py-3 px-4 max-w-md mx-auto gap-2">
          {[
            ...
            return (
              <button
                ...
                className={`group flex flex-col items-center py-2.5 px-3 rounded-2xl transition-all duration-300 ease-out min-w-[60px] flex-1 transform hover:scale-105 active:scale-95 active:bg-yellow-600 ${
                  activeTab === tab.id
                    ? 'text-gray-900 bg-yellow-400 shadow-lg shadow-yellow-500/40 border-2 border-yellow-500/80 ring-2 ring-yellow-300/50'
                    : 'text-gray-800 bg-yellow-400/80 hover:text-gray-900 hover:bg-yellow-400 hover:shadow-md hover:shadow-yellow-400/30 border-2 border-yellow-400/60 hover:border-yellow-500/70'
                }`}
              >
                <IconComponent 
                  size={20} 
                  className={`mb-1 transition-all duration-300 ease-out ${activeTab === tab.id ? 'drop-shadow-lg transform scale-110 filter brightness-110' : 'transform scale-100 group-active:scale-125'}`}
                  strokeWidth={activeTab === tab.id ? 2.8 : 2.2}
                />
                <span className={`text-[10px] font-bold leading-tight text-center transition-all duration-300 ease-out ${activeTab === tab.id ? 'transform scale-105 drop-shadow-sm' : 'transform scale-100'}`}>{tab.label}</span>
              </button>
            )
          })}
        </div>
