...
async function setupAuth(app2) {
  console.log("\u{1F510} Setting up Supabase authentication middleware");
  try {
...
var USDAService = class {
  ...
  async calculateIngredientCalories(ingredientName, measurement) {
    try {
      ...
      console.log(`\u{1F50D} Found ${foods2.length} foods for "${ingredientName}"`);
      const filteredFoods = this.filterAndPrioritizeFoods(foods2, ingredientName);
      if (filteredFoods.length === 0) {
        throw new Error("No suitable foods found");
      }
      const food = filteredFoods[0];
      console.log(`\u{1F4F1} Selected food: ${food.description} (ID: ${food.fdcId})`);
      console.log(`\u{1F4CA} Food has ${food.foodNutrients?.length || 0} nutrients`);
  ...
  extractNutrients(foodNutrients) {
    ...
    console.log(`\u{1F4CA} Extracting nutrients from ${foodNutrients?.length || 0} items`);
    ...
    for (const nutrient of foodNutrients) {
      ...
      console.log(`\u{1F50D} Nutrient: "${nutrient.nutrient.name}" = ${amount} ${nutrient.nutrient.unitName || ""}`);
      if (name === "energy" || name.includes("energy") || name.includes("calorie") || name.includes("kcal")) {
        nutrients.calories = amount;
        console.log(`\u{1F525} Found calories: ${amount}`);
   ...
   */
  filterAndPrioritizeFoods(foods2, searchTerm) {
    const searchLower = searchTerm.toLowerCase();
    const scoredFoods = foods2.map((food) => {
      let score = 0;
      ...
      if (description.includes("sauce") || description.includes("seasoning") || description.includes("mix") || description.includes("prepared")) {
        score -= 5;
      }
